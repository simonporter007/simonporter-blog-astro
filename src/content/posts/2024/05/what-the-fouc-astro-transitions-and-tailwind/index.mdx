...
You can see the site initially loads with the white theme in place, then the dark mode preference is read from storage, and so the class is applied and the theme changed. This causes the content to flicker as the styles are swapped out, and can happen during page transitions as well.
...
First, I'm going to assume you have an Astro project set up, and you're encountering this problem. If you're not using Astro, you can still follow along, but you'll need to adapt the code to your setup.

So in this case I'm using a React Astro island for my theme toggle. It attempts to load the current theme into state, and then a useEffect will adjust the document body class accordingly.

<Code code={ThemeToggleRaw} lang='tsx' title='ThemeToggle.tsx' />

and here he is! You can interact with this toggle to switch between light and dark mode. The theme is saved to local storage, so it will persist between page loads, and the class is applied to the document body to change the theme.
